...
const AssertionDisputedSelector =
  "0x033efc8167fe91406d16c1680e593c3c2cad864220645cc2a48efd67e8f7ca73";

const AssertionSettledSelector =
  "0x00ed635610f8859765fb89b19bec7866c02bbc2f03bb048acec6ba6536aa7cb9";
...
function decodeTransfersInBlock({ header, events }) {
  ...
  return events.flatMap(({ event, transaction }) => {
    if (event.keys[0] == AssertionMadeSelector) {
      ...
      claimData[claimData.length - 1] = trimLeadingZeros(
        claimData[claimData.length - 1],
      );
      const [
        assertionId,
        domainIdLow,
        domainIdHigh, // Skip the claim length as we've already used it
        ,
        ...rest
      ] = event.data;
